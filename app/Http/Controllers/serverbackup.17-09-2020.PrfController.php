    ...
    public function createPrf( Request $request){
        ...
        if( $request->isMethod('post')){
            $validation_array = [
                ...
                for($m=0; $m < $n ; $m++ ) {
                    ...
                    if( !empty($request->input('special_item_products.'.$m)) && !empty($request->input('special_item_product_quantity.'.$m)) )
                    {
                        ...
                        $order_snapshot[] = ["product_id"=>$request->input('special_item_products.'.$m),"product_name"=> $product->name(),"product_quantity"=>$request->input('special_item_product_quantity.'.$m),"product_price"=>$request->input('special_item_product_price.'.$m)];
            ...
            catch(Exception $e)
            {
                ...
                DB::rollback();
                $post_status = "FAILED";
                $post_status_message = "PRF generation failed";
                throw $e;

                $view_data['post_status'] = $post_status;
                $view_data['post_status_message'] = $post_status_message;
            
                return redirect('prf',$view_data);
            }
            DB::commit();
