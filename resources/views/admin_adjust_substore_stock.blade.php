      ...
      @include('includes.post_status')
        <div class="card">
            <div class="card-content">
                <p class="caption mb-0">{{$substore->name}} :  Inventory Adjustment
                    <!-- <a href="{{url('/substores/view-substore/'.$substore->id)}}" class=" pull-right  btn  waves-effect waves-light cyan darken-1 right" >Back
                    </a> -->
                </p>
            </div>
        </div>
        <div id="form-with-validation" class="card card card-default scrollspy col m12">
            <div class="card-content">
                <h4 class="card-title"> {{$product->name()}} : Current Inventory Qunatity: {{$substore->productInventory($product->id)}} </h4>
                <form action="{{url('admin/substore/inventory-adjustment/'.$substore->id.'/'.$product->id)}}" method="POST" >
                  {{csrf_field()}}
